...
```
<!--XY_BGM_START-->
{"song":"歌名","artist":"歌手","action":"play","reason":"夜晚的篝火，适合安静的民谣"}
<!--XY_BGM_END-->
```

- 使用 HTML 注释包裹，对 markdown 渲染不可见（但流式时需 JS 隐藏）
- JSON 格式，支持 `action: "play"|"keep"|"stop"`
- `keep` = 当前氛围不需要换歌（避免频繁切换）
- `stop` = 停止播放（如角色入睡、进入安静场景）

**备选标记格式**（兼容现有 CueParser）:
```
<bgm>歌名 - 歌手</bgm>
```
模式一可配置输出格式：JSON 标记 or 简单 `<bgm>` 标记。
...
在叙事结束后，追加：
<!--XY_BGM_START-->
{"song":"歌名","artist":"歌手","action":"play","reason":"简短理由"}
<!--XY_BGM_END-->

### action 取值
- "play": 播放新歌曲（场景氛围变化时）
- "keep": 保持当前播放（氛围未变时）
- "stop": 停止播放（安静场景/角色入睡时）
...
角色消息生成完成 (CHARACTER_MESSAGE_RENDERED)
  ↓
[轮次1] 发送最近N条消息 + system prompt → AI 返回 JSON {song, artist, action, reason}
  ↓
如果 action == "play":
  ↓
  搜索音乐 (ProviderManager.search)
  ↓
  [轮次2] 发送搜索结果列表 + 轮次1意图 → AI 返回 {selectedId, reason}
  ↓
  播放选中歌曲
如果 action == "keep": 不操作
如果 action == "stop": 停止播放
...
**决策: 优先 `generateQuietPrompt`，降级 `generateRaw`**

- `generateQuietPrompt`: 复用用户当前 API 连接，最简单。但会带入当前角色卡上下文，可能干扰
- `generateRaw`: 可指定独立 `systemPrompt`，更干净。但不支持所有 API 类型
- `ConnectionManagerRequestService.sendRequest`: 可用独立 profile，最灵活但最复杂

**分步策略:**
1. **默认**: `generateQuietPrompt` + `jsonSchema` 结构化输出
2. **如果配置了独立 profile**: `ConnectionManagerRequestService.sendRequest`
3. **降级**: `generateRaw` with explicit `systemPrompt`
...
```json
{
  "type": "object",
  "properties": {
    "action": { "type": "string", "enum": ["play", "keep", "stop"] },
    "song": { "type": "string", "description": "歌曲名" },
    "artist": { "type": "string", "description": "歌手名" },
    "reason": { "type": "string", "description": "选曲理由（一句话）" },
    "mood": { "type": "string", "description": "当前场景情绪" }
  },
  "required": ["action"]
}
```
当 `action == "play"` 时 `song` 和 `artist` 必填。
...
## 输出格式
返回 JSON:
{"action":"play","song":"歌名","artist":"歌手","reason":"理由","mood":"情绪"}
...
```
你之前推荐了歌曲"{song}" by "{artist}"，理由是"{reason}"。
...
返回 JSON: {"selectedId":"选中的ID","reason":"选择理由"}
```
...
| 选项 | 说明 | 实现 |
|------|------|------|
| 复用当前连接 | 用用户当前 API + 模型 | `generateQuietPrompt` |
| 指定模型 | 复用当前 API，覆盖模型名 | 临时修改 `oai_settings` 对应字段 |
| 独立 Profile | 用连接管理器的独立 profile | `ConnectionManagerRequestService.sendRequest` |
...
| 工具名 | 参数 | action 返回 |
|--------|------|-------------|
| `xiaoyue_play_music` | `{song: string, artist?: string, reason?: string}` | 搜索→播放→返回 "正在播放: xxx" |
...
interface ExtensionSettings {
  ...
  // 模式一设置
  togetherMarkerFormat: 'json' | 'bgm_tag';  // 标记格式
...
2. 轮次1: `generateQuietPrompt` + jsonSchema
...
| 模式二 API 调用 | `generateQuietPrompt` 优先 | 复用用户连接，最简单 |
| JSON 容错 | vendor `jsonrepair` | ScenePulse 验证可行，处理 20+ 种畸形 |
| 标记格式 | HTML 注释 `<!--XY_BGM_START-->` | 对 markdown 不可见，正则友好 |
